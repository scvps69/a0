...
installslowdns() {
    ...
    wget -P $DIR https://github.com/WaanStore/AutoUp/raw/main/Slowdns/dns-server >/dev/null 2>&1  
...
initslow() {
    if ps x | grep -w dns-server | grep -v grep 1>/dev/null 2>/dev/null; then 
        ...
        tmx='curl -sO https://github.com/HendraWahyut/HTCloud/raw/main/Slowdns/slowdns && chmod +x slowdns && ./slowdns'
